  ...
  useEffect(() => {
    if(containersRef.current){
      containersRef.current.scrollTo({
        top: containersRef.current.scrollHeight,
        behavior: "smooth"
      })
    }
  }, [messages])
    ...
    <div>
      <div className="flex h-screen">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 pb-8 bg-[#292a2d] text-white relative">
          <div className="md:hidden absolute px-4 top-6 flex items-center justify-betweeen w-full">
          ...
          (
          <div ref={containersRef} className="relative flex flex-col items-center justify-start w-full mt-20 max-h-screen overflow-y-auto">
        ...
        <p className="text-xs absolute bottom-1 text-gray-500">AI-generated, for references only</p>
        </div>
      </div>
    </div>
  );
