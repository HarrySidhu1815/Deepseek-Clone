...
const PromptBox = ({setIsLoading, isLoading}) => {
    ...
    const {user, setSelectedChat, selectedChat, chats, setChats} = useAppContext()
    ...
    const sendPrompt = async (e) => {
        ...
        try {
            ...
            //saving user prompt in chats arrays

            setChats((prevChat) => prevChat.map(chat => chat._id === selectedChat._id ? {
                ...chat,
                messages: [...chat.messages, userPrompt]
            } : chat))

            // saving user promptes in selected chat
  ...
  return (
    <form onSubmit={sendPrompt} className={`w-full ${selectedChat?.messages.length > 0 ? 'max-w-3xl' : 'max-w-2xl'} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
      <textarea onKeyDown={handleKeyDown} onChange={(e) => setPrompt(e.target.value)} value={prompt} className='outline-none w-full resize-none overflow-hidden break-words bg-transparent' rows={2} placeholder='Message Deepseek' required/>
